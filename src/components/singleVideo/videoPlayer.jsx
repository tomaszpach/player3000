...
export const VideoPlayer = ({ src, title }) => {
    ...
    if (src) {
        return (
            <div>
                <h1>{title}</h1>
                <figure onMouseEnter={(event) => handleOnHover(event, true)} onMouseLeave={(event) => handleOnHover(event, false)}>
                    <video  ref={videoRef} width="640" height="360" onClick={handlePlayPause}>
                        <source src={src} type="video/mp4"/>
                        Your browser does not support the video tag.
                    </video>
                    <figcaption>
                        <div className={`actions ${showControls ? 'visible' : 'hidden'}`}>
